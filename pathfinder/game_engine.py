...
from .types import PossibleMove, PLaygroundMarks
from .playground_printer import print_playground_with_current_move
...
def get_distance_from_target(target_point, move: PossibleMove) -> PossibleMove:
    distance = np.abs(move.move_in_x - target_point[0]) + np.abs(
        move.move_in_y - target_point[1]
    )
    return PossibleMove(move.move_in_x, move.move_in_y, distance, move.is_in_collision)


def check_move_for_collision_and_distance(
    move: PossibleMove, target_point, playground
) -> PossibleMove:
    collision_checked_move = is_move_in_collision(playground, move)
    distance_checked_move = get_distance_from_target(
        target_point, collision_checked_move
    )
    return distance_checked_move


def check_all_moves_for_collision_and_distance(
    target_point, playground, possible_moves: Iterator[PossibleMove]
) -> Iterator[PossibleMove]:
    return (
        check_move_for_collision_and_distance(move, target_point, playground)
...
def evaluate_next_progress(
    actual_point, target_point, playground, next_move: PossibleMove
) -> None:
    ...
    return find_path(
        (next_move.move_in_x, next_move.move_in_y), target_point, playground
    )
...
def find_path(actual_point, target_point, playground) -> None:
    ...
    checked_moves = check_all_moves_for_collision_and_distance(
        target_point, new_playground, possible_moves
    )
    filtered_moves = filter_feasible_moves(checked_moves)
    evaluate_next_progress(
        actual_point, target_point, new_playground, select_next_move(filtered_moves)
    )
