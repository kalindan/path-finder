...
from .types import PlaygroundParameters, PLaygroundMarks

def generate_empty_playground(x_size, y_size) -> list[list[str]]:
    return [[PLaygroundMarks.OBSTACLE if i == 0 or i == x_size-1 or j == 0 or j == y_size-1 else PLaygroundMarks.EMPTY_POINT for i in range(x_size)] for j in range(y_size)]
